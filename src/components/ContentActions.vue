<template>
    <v-container >
        <v-dialog v-model="show_dialog_new" 
                max-width="500px"
                persistent 
                no-click-animation
                transition=""
            ...
            <v-card>
                ...
                <v-btn color="blue darken-1"
                        text
                        @click="show_dialog_new = false">Cancel</v-btn>
                <v-btn color="blue darken-1"
                        text 
                        @click="addItem" 
                        :loading="loader_dialog_new" >Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>


        <v-dialog v-model="show_dialog_rename" 
                max-width="500px"
                persistent 
                no-click-animation
                transition=""
            ...
            <v-card>
                ...
                <v-container grid-list-md>
                    <v-layout wrap>
                    <v-flex xs12>
                        <v-text-field  label="Current Name" 
                                    readonly
                                    v-model="item_checkbox.title"
                        ></v-text-field>
                        <v-text-field  label="New Name"
                                    v-model="rename_new_name"
                        ></v-text-field>
                    </v-flex>
                    </v-layout>
                </v-container>
                ...
                <v-btn color="blue darken-1"
                        text
                        @click="show_dialog_rename = false">Cancel</v-btn>
                <v-btn color="blue darken-1"
                        text 
                        @click="renameItem" 
                        :loading="loader_dialog_rename" >Rename</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-card>
            ...
            <v-breadcrumbs :items="script_nav" >
                <template v-slot:item="props">
                    <v-breadcrumbs-item
                        @click="onClickNav(props.item.id)"
                        :href="'#navid=' + props.item.id"
            ...
            <v-btn
                ...
                dark
                class="mx-2"
                @click="show_dialog_new=true">New Action</v-btn>
          </v-card-title>
          <v-divider></v-divider>
         ...
         <v-card-text>
            ...
            <v-tabs v-if="show_editor" background-color="white" vertical v-model="current_tab">
                <v-tab href="#tab-raw">Content</v-tab>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            item_checkbox: {title: ''},
            script_nav: [  {text: 'root', id: "00", disabled: false} ],
            items_list: [],
            script_listing: [],
            show_listing: true,
            show_editor: false,
        ...
        watch: {
            current_workspace(){
                ...
                // reset the tree
                this.items_list = []
                this.script_nav =[  {text: 'Actions', id: "00", disabled:false, type: "root"} ]
        ...
        methods: {
            ...
            async runAction(){
                ...
                await BackendApi.scheduleJobContent(this.current_workspace, this.file_raw).then(resp => {
                    if ( resp != undefined) {
                        this.job_id = null,
                        this.logs_index = 0
                        this.raw_logs = []
            ...
            async saveChanges(){
                ...
                // save file
                await BackendApi.addAction(path_str, this.current_workspace, this.file_raw).then(resp => {
                    if ( resp != undefined) {
            ...
            async addItem(){
                ...
                var content_file = ""
                // rename item
                await BackendApi.addAction(path_str, this.current_workspace, content_file)
            ...
            async duplicateItem(){
                ...
                // delete item
                await BackendApi.duplicateAction(path_str, this.current_workspace)
            ...
            async renameItem(){
                ...
                // rename item
                await BackendApi.renameAction(path_str, this.current_workspace, this.rename_new_name)
            ...
            async deleteItem(){
                ...
                // delete item
                await BackendApi.deleteAction(path_str, this.current_workspace)
            ...
            async onClickFile(id){
                ...
                // load the file
                await BackendApi.getActionFile(path_str, this.current_workspace).then(resp => {
                    if ( resp != undefined) {
                            this.file_raw = resp.actions.file.content
                            this.is_ready = true
            ...
            onClickFolder(id){
                ...
                // update the navigation menu of the tree
                this.script_nav.push( {text: this.script_listing[id].name, id: id, disabled:false}  )
            ...
            getActionsListing(){
                ...
                this.script_nav = [ {text: 'Actions', id: "00", disabled: false} ]
                this.items_list = []
        ...
        async created () {
            ...
            EventBus.$emit('CurrentPageChanged', { name: "Actions", position: 2 } )
